...
NETRA can wire a local [ChromaDB](https://www.trychroma.com/) vector
...
switch in `netra.toml`. With the switch off, the rest of the app is
unaffected.
...
1. A managed Python venv under `~/.netra/chroma-venv` with
...
3. An `mcpServers.chroma` entry added to `~/.pi/agent/config.json` (kept for
   ...
   the agent reaches chroma via the `netra-chroma` CLI instead; see
   [How the agent uses ChromaDB](#how-the-agent-uses-chromadb)).
4. A per-project collection (`netra_<project_id>`) populated from the project
...
1. **First time only** — clicking *Install* in the ChromaDB pane does:
   ...
   - `uv venv ~/.netra/chroma-venv --python 3.11`.
...
2. **Every launch** (when `enabled = true` and `auto_start = true`):
   ...
   - registers `mcpServers.chroma` in `~/.pi/agent/config.json` (first edit
     backs the file up to `config.json.netra-bak`),
...
~/.netra/chroma-venv ~/.netra/chroma-data` plus deleting the `chroma`
...
host                 = "127.0.0.1"
port                 = 8765
data_dir             = "/home/you/.netra/chroma-data"
auto_start           = true
...
restart (the runtime handle is constructed during `Netra::start`).

---

## Embedding model

Chroma 1.x requires client-side embeddings, so NETRA runs a small
...
returns the list of `netra_*` collections currently on the server. The UI
...
| `chroma_index_project`           | Walk + chunk + upsert; streams `netra://chroma` events |
| `chroma_update_settings`         | Save `ChromaConfig` to `netra.toml`           |
| `chroma_set_enabled`             | Toggle the master switch (restart required)    |
| `chroma_set_embedding_model`     | Persist `embedding_model` (+ optional custom HF id); returns whether reindex is needed and which `netra_*` collections are affected |
| `chroma_drop_and_reindex`        | Drop the listed project collections and re-index each (streams `netra://chroma` events) |
...
`netra_<project_id>` before answering. When the server is not running,
the flag is a silent no-op.

The frontend listens on the `netra://chroma` event channel for
...
ship a tiny CLI — `netra-chroma` — that the agent invokes through its
existing `bash` tool.

When you queue a job (or batch) with **Use ChromaDB** ticked AND the server
is running, NETRA:

1. Writes `~/.netra/netra_chroma_cli.py` + the `~/.netra/bin/netra-chroma`
   shim (idempotent — content-diff check).
2. Sets these env vars on the spawned `pi` process:
   `NETRA_CHROMA_HOST`, `NETRA_CHROMA_PORT`, `NETRA_CHROMA_MODEL`,
   `NETRA_CHROMA_COLLECTION`.
...
`--collection` defaults to `NETRA_CHROMA_COLLECTION`. Add `--json` to any
...
The full prompt text lives in `crates/netra-chroma/src/prompt.rs` —
`chromadb_system_prompt(collection_name, shim_path)`. The resolver
(`crates/netra-api/src/resolver.rs`) appends it on every job whose
...
1. Run 2–4 `netra-chroma query` calls with different short phrasings of the
...
```
"Where is request authentication handled? Use netra-chroma query first,
 then read the top hits."

"Summarise the SQLite schema and its migrations — run netra-chroma query
 with 'migration' and 'CREATE TABLE' before reading."

"Find every place that calls run_reviewer; netra-chroma query with
...
Backed by `chroma_query` Tauri command → `netra_chroma::query::query_collection`
...
3. Edit `~/.pi/agent/config.json` and delete the `mcpServers.chroma` entry
   (restore from `config.json.netra-bak` if you prefer).
4. Optional cleanup: `rm -rf ~/.netra/chroma-venv ~/.netra/chroma-data`.

After step 1, restarting NETRA runs zero ChromaDB code — the integration
imposes no overhead.
