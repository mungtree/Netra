//! ChromaDB integration for Mini ChatUR (opt-in).
//!
//! This crate is only compiled when the `chromadb` feature is enabled on
//! `chatur-api` (or when depended on directly, e.g. by the Tauri shell).
...
///
/// Lives under `[chromadb]` in `chatur.toml`. When `enabled = false` (the
...
#[must_use]
pub fn chatur_runtime_dir() -> PathBuf {
    dirs::home_dir()
        .unwrap_or_else(|| PathBuf::from("."))
        .join(".chatur")
}
