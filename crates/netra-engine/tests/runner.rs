...
//!
//! These wire the real `netra-store` SQLite repositories and the
//! `netra-agent` mock transport together — no `pi` process is spawned.
...
use netra_agent::{AgentPool, AgentSpec, MockTransport, MockTransportFactory, TransportFactory};
use netra_core::ids::JobId;
use netra_core::model::{AgentEvent, Job, JobStatus, Project};
use netra_core::traits::{AgentTransport, DomainEvent, EventBus, JobQueue, JobRepo, ProjectRepo};
