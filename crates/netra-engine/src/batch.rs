...
use netra_core::ids::{BatchId, JobId};
...
impl BatchExecutor {
    ...
    async fn execute(&self, batch: &Batch) -> Result<AggregatedResult> {
        ...
        let mut project_cache: HashMap<_, netra_core::model::Project> = HashMap::new();
    ...
    /// generates a [`FindingsReport`](netra_core::model::FindingsReport)
...
fn parse_findings(text: &str) -> Option<netra_core::model::FindingsReport> {
