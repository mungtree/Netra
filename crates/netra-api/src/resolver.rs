...
use netra_chroma::{chromadb_system_prompt, ChromaConfig, ChromaHandle};
use netra_core::Result;
use netra_core::model::{Job, ModelRef};
use netra_core::traits::{DomainEvent, EventBus, ProjectRepo};
use netra_engine::SpecResolver;
use netra_store::SqliteProjectRepo;
...
/// (one knob in `netra.toml` rather than per-project).
...
impl SpecResolver for ProjectSpecResolver {
    async fn resolve(&self, job: &Job) -> Result<AgentSpec> {
        ...
        if let Some(name) = &job.module_name {
            ...
            spec = spec.with_env("NETRA_MODULE_ROOT", module_root.display().to_string());
        }
        if job.use_chromadb {
            match &self.chroma {
                ...
                Some(chroma) => {
                    if !chroma.is_running().await {
                        ...
                        match netra_chroma::bootstrap::ensure_shim() {
                            Ok(shim) => {
                                appended.push(chromadb_system_prompt(&coll, &shim));
                                spec = spec
                                    .with_env("NETRA_CHROMA_HOST", cfg.host.clone())
                                    .with_env("NETRA_CHROMA_PORT", cfg.port.to_string())
                                    .with_env("NETRA_CHROMA_MODEL", cfg.resolved_model())
                                    .with_env("NETRA_CHROMA_COLLECTION", coll.clone());
                            }
                            Err(e) => {
...
mod tests {
    use super::*;
    use netra_core::model::{Job, Project};
    use netra_engine::BroadcastEventBus;
    ...
    async fn module_block_present_when_job_is_scoped() {
        ...
        assert_eq!(
            spec.env.get("NETRA_MODULE_ROOT").map(String::as_str),
            Some("/tmp/p/server")
        );
    }
    ...
    async fn module_block_absent_for_unscoped_job() {
        ...
        assert!(!spec.env.contains_key("NETRA_MODULE_ROOT"));
    }
}
