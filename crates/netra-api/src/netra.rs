//! [`Netra`] — the facade that wires the store, agent pool, and engine.
...
use netra_agent::{AgentPool, PiTransportFactory, TransportFactory};
use netra_chroma::{ChromaHandle, ChromaStatus};
use netra_core::ids::{BatchId, JobId, ProjectId};
use netra_core::model::{Batch, BatchBuilder, BatchItem, Job, JobStatus, Project};
use netra_core::traits::{
...
use netra_store::{Database, FileLogSink, SqliteJobQueue};

use crate::config::{NetraConfig, ModelConfig};
use crate::notify;
use crate::planner_supervisor::{PlannerRuntimeConfig, PlannerSupervisor};
use crate::resolver::ProjectSpecResolver;

/// The NETRA application: one running instance owns the database, the
/// agent pool, the job queue, the event bus, and a background scheduler.
///
/// Construct with [`Netra::start`]; tear down with [`Netra::shutdown`].
pub struct Netra {
    config: NetraConfig,
    db: Database,
...
impl Netra {
    ...
    pub async fn start(config: NetraConfig) -> Result<Self> {
    ...
    pub async fn start_with_factory(
        ...
        let db = Database::connect(config.data_dir.join("netra.db")).await?;
        ...
        let chroma = if config.chromadb.enabled {
            ...
            if config.chromadb.auto_start {
                ...
                tokio::spawn(async move {
                    ...
                    if let Err(e) = netra_chroma::bootstrap::ensure_venv().await {
                    ...
                    if let Err(e) = netra_chroma::mcp::register_pi_mcp(&cfg.host, cfg.port) {
                        tracing::warn!("failed to register chroma MCP entry: {e}");
                    }
                    if let Err(e) = netra_chroma::server::start(&h2).await {
    ...
    #[must_use]
    pub fn config(&self) -> &NetraConfig {
        &self.config
    }
    ...
    pub async fn create_batch_full(
        ...
        for spec in targets {
            let mut target = netra_core::model::BatchTarget::project(spec.project_id);
    ...
    async fn module_map(
        &self,
        batch: &Batch,
    ) -> Result<std::collections::HashMap<ProjectId, Vec<netra_core::ids::ModuleId>>> {
    ...
    pub async fn shutdown(self) -> Result<()> {
        ...
        if let Some(chroma) = &self.chroma {
            let _ = netra_chroma::server::stop(chroma).await;
        }
        self.planner_supervisor.shutdown().await;
        tracing::info!("Netra stopped");
        Ok(())
    }
}

/// One target for [`Netra::create_batch_full`]: a project plus an optional
...
pub struct BatchTargetSpec {
    ...
    pub module_ids: Option<Vec<netra_core::ids::ModuleId>>,
}
...
async fn orphan_failure(
    projects: &netra_store::SqliteProjectRepo,
    job: &Job,
    module_still_exists: &mut impl FnMut(&Project, netra_core::ids::ModuleId) -> bool,
...
async fn fail_job(
    jobs: &netra_store::SqliteJobRepo,
    job: &mut Job,
    reason: String,
) -> Result<()> {
