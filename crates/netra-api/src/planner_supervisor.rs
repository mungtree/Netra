//! Supervisor for the Python `netra-planner` sidecar.
//!
//! Spawns `python -m netra_planner.server` (or `uvicorn` directly) on startup,
...
/// Subset of [`crate::config::NetraConfig`] that the supervisor needs.
#[derive(Debug, Clone)]
...
impl PlannerSupervisor {
    ...
    pub async fn start(&self, cfg: &PlannerRuntimeConfig) -> Result<(), PlannerError> {
        ...
        cmd.arg("-m").arg("netra_planner.server");
        cmd.current_dir(&sidecar_dir);
        cmd.env("NETRA_PLANNER_PORT", port.to_string());
        if let Some(model) = cfg.default_model.as_ref() {
            cmd.env("NETRA_PLANNER_MODEL", &model.model);
            if let Some(url) = model.base_url.as_ref() {
                cmd.env("NETRA_PLANNER_LLAMACPP_URL", url);
            }
        }
        ...
        let mut child = cmd.spawn().map_err(|e| {
            PlannerError::Spawn(format!(
                "spawn `{} -m netra_planner.server` in `{}`: {e}",
                python.display(),
                sidecar_dir.display()
            ))
        })?;
    ...
    /// Kill the child (if any). Called on `Netra::shutdown`.
    ...
    R: tokio::io::AsyncRead + Unpin + Send + 'static,
{
    ...
    while let Ok(Some(line)) = lines.next_line().await {
        tracing::info!(target: "netra_planner", "{label}: {line}");
    }
}
