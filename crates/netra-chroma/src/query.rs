...
pub async fn query_collection(
    ...
    let mut child = cmd.spawn().map_err(|e| {
        ...
        tracing::error!(target: "netra::chroma::query", "{msg}");
        ChromaError::Query {
            stage: "spawn".into(),
            message: msg,
            stderr: None,
    ...
    if let Some(mut stdin) = child.stdin.take() {
        if let Err(e) = stdin.write_all(&payload).await {
            tracing::error!(target: "netra::chroma::query", "query helper stdin: {e}");
            return Err(ChromaError::Query {
                stage: "stdin".into(),
        ...
        if let Err(e) = stdin.shutdown().await {
            tracing::error!(target: "netra::chroma::query", "query helper stdin close: {e}");
            return Err(ChromaError::Query {
                stage: "stdin".into(),
    ...
    let out = child.wait_with_output().await.map_err(|e| {
        tracing::error!(target: "netra::chroma::query", "query helper wait: {e}");
        ChromaError::Query {
            stage: "wait".into(),
