...
/// expose chroma through a small CLI (`netra-chroma`) that the agent invokes
...
the `netra-chroma` CLI using your existing `bash` tool. There are NO \
...
`NETRA_CHROMA_COLLECTION` env var, so you can usually omit it.\n\
\n\
...
\n\
If a `netra-chroma` invocation fails, read the stderr verbatim and fix the\n\
