...
Env (set by the netra-chroma rust shim or by the resolver per pi spawn):

    NETRA_CHROMA_HOST        defaults to 127.0.0.1
    NETRA_CHROMA_PORT        defaults to 8765
    NETRA_CHROMA_MODEL       defaults to "default" (must match the indexer
                              that built the collection or query fails on dim
                              mismatch)
    NETRA_CHROMA_COLLECTION  optional default for --collection

Subcommands:

    netra-chroma query   --query <text> [--collection <name>] [--n 10] [--where <json>]
    netra-chroma peek    [--collection <name>] [--n 5]
    netra-chroma list
    netra-chroma info    [--collection <name>]
    netra-chroma get     [--collection <name>] [--ids id1,id2] [--where <json>]
...
def resolve_collection(arg: str | None) -> str:
    if arg:
        return arg
    env = os.environ.get("NETRA_CHROMA_COLLECTION")
    if env:
        return env
    print(
        "error: --collection not given and NETRA_CHROMA_COLLECTION not set",
        file=sys.stderr,
    )
    sys.exit(64)
...
def open_client_and_collection(collection: str):
    host = os.environ.get("NETRA_CHROMA_HOST", "127.0.0.1")
    port = int(os.environ.get("NETRA_CHROMA_PORT", "8765"))
    model = os.environ.get("NETRA_CHROMA_MODEL", "default")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="netra-chroma")
...
def main() -> int:
    ...
    try:
        ...
        print(f"netra-chroma error: {e}", file=sys.stderr)
        return 1
